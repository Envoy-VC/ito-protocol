...
contract ItoProxy {
    ...
    constructor(address _contractOwner, address _diamondCutFacet, address _ownershipFacet) payable {
        ...
        ItoProxyLib.diamondCut(cut, address(0), "");
    }
    ...
    fallback() external payable {
        ItoProxyLib.DiamondStorage storage ds;
        bytes32 position = ItoProxyLib.DIAMOND_STORAGE_POSITION;
        // get diamond storage
        assembly {
            ds.slot := position
        }
        ...
        require(facet != address(0), "ItoProxy: Function does not exist");
